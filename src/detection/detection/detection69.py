...
class ColorDetectionNode(Node):
    ...
    def process_image(self, msg):
        ...
        self.detect_cones(cv_image, cv_image_resized)  
    ...
    def detect_rectangle(self, image):
        ...
        best_approx, max_area = self.find_best_rectangle(image, 1200) #CHANAGED HERE!!!
    ...
    # is_triangle_like = vertex_count <= 7
        ...
        if not is_tall:
            self.get_logger().info("Failed aspect ratio test")
    ...
    def detect_cones(self, original_image, cv_image_resized):
        ...
        for box in result.boxes:
            ...
            if conf >= 0.80:
                ...
                if area > 5000 and area < 500000: # BUMP UP TO TUNED SIZE!!!!
 
                    bb_count += 1
 
                    bbox_list.append([bbox,area])
        ...
        if max_area>0:
            ...
            self.get_logger().info(f"Biggest cone: {biggest_box}")
                # Add label
            label = f"B CONE {max_area:.2f}"
            cv2.putText(image, label, (x1, y2 + 40), cv2.FONT_HERSHEY_SIMPLEX,
                        1.5, (0, 255, 0), 4)
        
            # white out biggest cone
            self.get_logger().info("Checking if a bigger bucket exists")
        ...
        image_obj, bbox_object = self.detect_rectangle(remove_bb)

        if bbox_object is not None:
            
            # object found
            self.get_logger().info("Found a bucket.. checking sizes")
            ...
            if area_object > max_area:
                # return bbox coordinates of bucket
                self.get_logger().info("Bucket is bigger!")
                image = image_obj
                self.object_bb = bbox_object

            else:
                # check cone is big enough or None
                if max_area > 20000:
                    self.get_logger().info("Big enough cone detected")
                    self.object_bb = biggest_box
                
                else:
                    self.get_logger().info("Cone too small or not detected")
                    self.object_bb = None

        else:
            self.get_logger().info("No bucket detected...")
